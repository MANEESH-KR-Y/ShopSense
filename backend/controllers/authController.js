...
exports.login = async (req, res) => {
  try {
    ...
    let user = email ? await User.findByEmail(email) : await User.findByPhone(phone);
  ...
  try {
    ...
    if (!isValid) {
      ...
      const check = await require('../db').query(
        'SELECT otp, otp_expires_at, NOW() as now FROM users WHERE id=$1',
        [user.id]
      );
...
exports.token = async (req, res) => {
  try {
    ...
    if (!user) return res.status(403).json({ error: 'Refresh token not in DB' });

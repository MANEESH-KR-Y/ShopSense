    ...
    <div className="flex h-screen bg-[var(--color-brand-black)] text-[var(--color-brand-text)] font-sans overflow-hidden">
      ...
      <main className="flex-1 p-8 overflow-y-auto relative bg-[var(--color-brand-black)]">
        ...
        <h1 className="text-3xl font-bold mb-8 text-white border-b border-[var(--color-brand-border)] pb-4 relative z-10 flex justify-between items-center">
          ...
          <button
            onClick={() => (window.location.href = '/analytics/report')}
            className="bg-[var(--color-brand-blue)] hover:bg-blue-600 text-white text-sm px-4 py-2 rounded uppercase font-bold transition-colors"
          >
            📄 Print GST Report
          </button>
        </h1>
              ...
              <div className="flex flex-wrap gap-4 mb-6 p-4 bg-[var(--color-brand-black)] border border-[var(--color-brand-border)] rounded-lg">
                ...
                <div className="flex gap-4 items-center">
                  ...
                  <button
                    ...
                    className="bg-[var(--color-brand-blue)] text-white px-4 py-2 rounded font-bold hover:bg-blue-600 transition-colors"
                  >
                    Analyze 🔎
                  </button>
                </div>
              ...
              {salesAnalysis && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="p-6 bg-[var(--color-brand-surface)] border border-[var(--color-brand-border)] rounded-lg flex items-center justify-between">
                    <div>
                      <p className="text-[var(--color-brand-text-muted)] text-sm uppercase">
                        Total Revenue
                      </p>
                      <p className="text-3xl font-bold text-green-400">
                        ₹{parseFloat(salesAnalysis.total_sales || 0).toFixed(2)}
                      </p>
                    </div>
                ...
                {stats?.topSelling?.length > 0 ? (
                  <ul className="space-y-4">
                    {stats.topSelling.map((item, i) => (
                      <li
                        key={i}
                        className="flex justify-between items-center p-3 bg-[var(--color-brand-surface)] border border-[var(--color-brand-border)] rounded hover:border-green-500/50 transition-colors"
                      >
                ...
                {stats?.lowSelling?.length > 0 ? (
                  <ul className="space-y-4">
                    {stats.lowSelling.map((item, i) => (
                      <li
                        key={i}
                        className="flex justify-between items-center p-3 bg-[var(--color-brand-surface)] border border-[var(--color-brand-border)] rounded hover:border-blue-500/50 transition-colors"
                      >
                ...
                {stats?.lowStock?.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {stats.lowStock.map((item, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-center p-4 bg-[var(--color-brand-black)] border border-red-500/30 rounded hover:bg-red-900/10 transition-colors"
                      >
                        <div>

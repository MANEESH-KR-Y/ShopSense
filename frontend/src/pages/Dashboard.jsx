      ...
      <main className="flex-1 p-8 overflow-y-auto relative bg-[var(--color-brand-black)]">
        ...
        <header className="flex justify-between items-center mb-10 relative z-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <div
              className="w-10 h-10 rounded-full bg-[var(--color-brand-surface)] border border-[var(--color-brand-border)] flex items-center justify-center cursor-pointer hover:border-[var(--color-brand-blue)] transition-colors"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10 relative z-10">
          {stats.map((stat, index) => (
            <div
              key={index}
              className="card hover:border-[var(--color-brand-blue)] transition-all duration-300 hover:shadow-lg hover:shadow-blue-900/10 group"
            >
              <h3 className="text-[var(--color-brand-text-muted)] text-xs font-bold mb-3 uppercase tracking-widest">
                {stat.title}
              </h3>
              {loading ? (
                ...
                <p
                  className={`text-3xl font-bold ${stat.colorClass} group-hover:scale-105 transition-transform origin-left`}
                >
                  {stat.value}
                </p>
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
          <div className="card lg:col-span-2">
            <h3 className="text-lg font-bold text-white mb-6 border-b border-[var(--color-brand-border)] pb-4 flex items-center justify-between">
              ...
              <button className="text-xs text-[var(--color-brand-blue)] hover:text-white transition-colors">
                View All
              </button>
            </h3>
            <div className="space-y-4">
              {loading ? (
                [1, 2, 3].map((_, i) => (
                  <div
                    key={i}
                    className="h-16 bg-[var(--color-brand-border)]/50 rounded animate-pulse"
                  ></div>
                ))
                ...
                recentActivity.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center gap-4 p-4 border border-[var(--color-brand-border)] hover:border-[var(--color-brand-blue)] rounded-lg transition-all bg-[var(--color-brand-surface)]/50 hover:bg-[var(--color-brand-surface)]"
                  >
                    ...
                    <div>
                      <p className="font-medium text-white text-sm">
                        {item.type}{' '}
                        <span className="text-[var(--color-brand-text-muted)]">#{item.id}</span>
                      </p>
                      <p className="text-xs text-[var(--color-brand-text-muted)]">{item.time}</p>
                    </div>
                    {item.amount && item.amount !== '-' && (
                      <span className="ml-auto font-bold text-green-400 text-sm">
                        {item.amount}
                      </span>
                    )}
                  </div>
                ...
                <div className="text-center py-10 text-[var(--color-brand-text-muted)]">
                  No recent activity
                </div>
              )}
            </div>
          ...
          <div className="card h-fit">
            <h3 className="text-lg font-bold text-white mb-6 border-b border-[var(--color-brand-border)] pb-4">
              Quick Actions
            </h3>
            <div className="grid grid-cols-2 gap-4">
              <button
                onClick={() => navigate('/inventory/add')}
                className="btn btn-secondary py-6 flex-col gap-2 hover:border-[var(--color-brand-blue)] hover:text-[var(--color-brand-blue)] group"
              >

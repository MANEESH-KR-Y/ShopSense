...
export default function NetworkStatus() {
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 px-4 py-2 rounded-full font-bold shadow-lg z-50 flex items-center gap-2 transition-all
      ${
        isOnline
    ...
    >
      ...
      {isOnline && queue.length > 0 && (
        <>
          ...
          <span>
            {isSyncing ? 'SYNCING...' : 'PENDING SYNC'} ({queue.length})
          </span>
        </>
      )}
    </div>

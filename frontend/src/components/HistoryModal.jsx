...
export default function HistoryModal({ isOpen, onClose }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className={`bg-[var(--color-brand-surface)] border border-[var(--color-brand-border)] rounded-lg shadow-2xl w-full flex flex-col max-h-[90vh] transition-all ${selectedOrder ? 'max-w-6xl' : 'max-w-4xl'}`}>
                ...
                <div className="flex justify-between items-center p-6 border-b border-[var(--color-brand-border)]">
                    ...
                    <div className="flex gap-4">
                        {selectedOrder && (
                            <button onClick={() => setSelectedOrder(null)} className="text-sm bg-gray-700 px-3 py-1 rounded text-white hover:bg-gray-600">
                                ← Back to List
                            </button>
                        )}
                        <button onClick={onClose} className="text-white hover:text-red-500 text-3xl leading-none">&times;</button>
                    </div>
                </div>

                <div className="flex-1 overflow-hidden flex">
                    {/* List View */}
                    <div className={`flex-1 overflow-auto p-6 custom-scrollbar ${selectedOrder ? 'hidden md:block md:w-1/3' : 'w-full'}`}>
                        {loading ? (
                            <div className="text-center text-[var(--color-brand-text-muted)] animate-pulse">Loading...</div>
                        ) : history.length === 0 ? (
                            <div className="text-center text-[var(--color-brand-text-muted)]">No orders found.</div>
                        ) : (
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="text-white">
                                    {history.map(order => (
                                        <tr key={order.id} className={`border-b border-[var(--color-brand-border)] hover:bg-[var(--color-brand-black)]/50 transition-colors ${selectedOrder?.id === order.id ? 'bg-[var(--color-brand-blue)]/10' : ''}`}>
                                            <td className="py-4 pl-2 font-mono text-[var(--color-brand-blue)]">#{order.id}</td>
